    ...
    <div>
      <div style="position:sticky;top:92px;display:flex;flex-direction:column;gap:20px;">
        ...
        <div style="background:var(--c-surface);border:1px solid var(--g-border);border-radius:var(--r-xl);padding:28px;" class="reveal">
          ...
          <button onclick="messageProvider()" class="btn-outline" style="width:100%;justify-content:center;padding:12px;font-size:12px;">
...
function messageProvider() {
  var pid = window.location.pathname.split('/').pop();
  if (!pid) return;
  var tok = localStorage.getItem('sl_token') || localStorage.getItem('token');
  if (!tok) { window.location.href = '/login?redirect=' + encodeURIComponent('/messages?provider_id=' + pid); return; }
  window.location.href = '/messages?provider_id=' + pid;
}
