export const baseHead = (title: string, extra = '') => `
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .container{width:100%;max-width:1200px;margin:0 auto;padding:0 20px}@media(max-width:767px){body{font-size:14px;padding-bottom:100px}.container{padding:0 16px}.hide-mob{display:none!important}}@media(min-width:768px){.hide-desk{display:none!important}}
    ...
    .nav-main{position:fixed;top:0;left:0;right:0;z-index:1000;background:rgba(255,255,255,.94);backdrop-filter:blur(28px);border-bottom:1px solid var(--i-faint)}.nav-link{color:var(--t-secondary);text-decoration:none;font-size:13px;font-weight:500;padding:8px 14px;border-radius:100px}.nav-link:hover,.nav-link.active{color:var(--g-main);background:var(--g-dim)}.mobile-nav{position:fixed;bottom:16px;left:50%;transform:translateX(-50%);width:calc(100% - 32px);max-width:420px;background:rgba(255,255,255,.96);border:1px solid rgba(255,255,255,.6);border-radius:28px;padding:8px;display:flex;justify-content:space-around;gap:4px;z-index:900;box-shadow:0 8px 40px rgba(0,0,0,.16)}.mobile-nav-item{flex:1;display:flex;flex-direction:column;align-items:center;gap:3px;padding:8px 4px;border-radius:20px;color:var(--t-muted);text-decoration:none;font-size:9px;font-weight:600}.mobile-nav-item.active{color:var(--g-main);background:var(--g-dim)}.mobile-nav-item.primary{background:var(--g-main);color:#fff;margin-top:-28px;width:64px;height:64px;flex:0 0 64px;border-radius:50%;justify-content:center}@media(min-width:768px){.mobile-nav{display:none}}
