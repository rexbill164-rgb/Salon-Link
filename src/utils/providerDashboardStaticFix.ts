...
(function(){
  ...
  function loadServices(){ var x=a(), e=q('my-services-list'); if(!x||!e)return; x.get('/api/providers/me/services',{headers:h()}).then(function(r){ var rows=(r.data||{}).services||[]; if(!rows.length){e.innerHTML='<div style="text-align:center;color:var(--t-muted);padding:20px;font-size:12px;">No services yet. Add your first service above.</div>';return;} e.innerHTML=rows.map(function(s){return '<div style="display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid var(--i-faint);"><div style="flex:1"><div style="font-size:13px;font-weight:700;">'+(s.name||'Service')+'</div><div style="font-size:11px;color:var(--t-muted);">'+(s.duration_minutes||60)+' min</div></div><div style="font-size:15px;font-weight:700;color:var(--g-main);">'+money(s.price)+'</div></div>';}).join(''); }).catch(function(){e.innerHTML='<div style="text-align:center;color:var(--t-muted);padding:20px;font-size:12px;">Could not load services.</div>';}); }
