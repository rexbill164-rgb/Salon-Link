...
function userId(user: any): number {
...
function makeConversationId(providerId: number, customerId: number): string {
  return `provider:${providerId}:customer:${customerId}`
}
...
async function providerForUser(c: any, providerUserId: number) {
  return await c.env.DB.prepare(
    'SELECT id, user_id, business_name FROM providers WHERE user_id = ?'
  ).bind(providerUserId).first() as any
}

async function providerById(c: any, providerId: number) {
  return await c.env.DB.prepare(
    'SELECT id, user_id, business_name FROM providers WHERE id = ?'
...
async function bookingBelongsToConversation(c: any, bookingId: number, providerId: number, customerId: number) {
  const booking = await c.env.DB.prepare(
    'SELECT id FROM bookings WHERE id = ? AND provider_id = ? AND customer_id = ?'
  ).bind(bookingId, providerId, customerId).first()
  return !!booking
}
...
messages.post('/send', async (c) => {
  try {
    ...
    if (user.role === 'provider') {
      ...
      if (requestedProviderId && Number(provider.id) !== requestedProviderId) {
        return c.json({ success: false, error: 'Cannot send messages for another provider' }, 403)
      }
      ...
      ).bind(requestedReceiverId).first() as any
      ...
      if (requestedReceiverId && requestedReceiverId !== receiverId) {
        return c.json({ success: false, error: 'Receiver does not match provider' }, 403)
      }
    }
  ...
  try {
    ...
    const result = await retryWithSchema(c, async () => await c.env.DB.prepare(`
      SELECT latest.conversation_id, latest.id as last_message_id, latest.sender_id, latest.receiver_id,
        ...
        other.id as other_user_id, other.first_name as other_first_name, other.last_name as other_last_name,
